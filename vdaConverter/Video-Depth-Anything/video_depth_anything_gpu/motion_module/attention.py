...
class CrossAttention(nn.Module):
    ...
    def _attention(self, query, key, value, attention_mask=None):
        # Degenerate single-key case: with a sequence length of 1 the softmax
        # runs over a single logit, which is exactly 1.0 for any input, so
        # attention_probs @ value == value. This is the first-frame path of
        # the init signature, where no cache exists yet.
        #
        # Skipping the baddbmm/softmax/bmm chain here is an exact
        # simplification, not an approximation -- and it removes a lowered
        # `y / y` division that the TFLite GPU delegate refuses outright
        # ("DIV: No support of few identical inputs in the same operation"),
        # which otherwise kills delegate init for the whole graph.
        if attention_mask is None and query.shape[1] == 1 and key.shape[1] == 1:
            return self.reshape_batch_dim_to_heads(value)
...
class FeedForward(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # Final projection applied as an equivalent 1x1 convolution rather
        # than through nn.Linear. Same reason as TemporalAttention's
        # to_out[0] (see motion_module.py): nn.Linear lowers to TFLite's
        # FULLY_CONNECTED, which flattens leading dims to a 2D [N, C]
        # output. That 2D result is then added to the 3D [1, N, C] residual
        # stream in TemporalTransformerBlock, and the GPU delegate's ADD
        # kernel rejects the rank mismatch. CONV_2D does not flatten, so the
        # tensor stays rank 4 with batch=1.
        #
        # The weights are the very same nn.Linear parameters, just viewed as
        # [c_out, c_in, 1, 1], so self.net[-1] remains an nn.Linear and the
        # checkpoint's state_dict keys are untouched.
        linear = self.net[-1]
        ...
        return y4.permute(0, 2, 3, 1).reshape(n, f, c_out)
